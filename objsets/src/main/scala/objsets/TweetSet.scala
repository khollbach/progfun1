...
abstract class TweetSet {
  ...
  def mostRetweetedHelper(bestSoFar: Tweet): Tweet
...
class Empty extends TweetSet {
  ...
  def mostRetweetedHelper(bestSoFar: Tweet): Tweet = bestSoFar
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweetedHelper(bestSoFar: Tweet): Tweet = {
    def better(t1: Tweet, t2: Tweet): Tweet =
      if (t1.retweets > t2.retweets) t1 else t2

    left.mostRetweetedHelper(
      right.mostRetweetedHelper(
        better(elem, bestSoFar)))
  }
